#include "aes.h"
#include <stdlib.h>
#include <stdio.h>

/*Definiton*/
uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE] = {};
uint8_t roundkey[STATE_ROW_SIZE][STATE_COL_SIZE] = {};

const uint8_t rcon[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

const uint8_t mixColumnMatrix[STATE_ROW_SIZE][STATE_COL_SIZE] = {{0x02, 0x03, 0x01, 0x01},
                                                                 {0x01, 0x02, 0x03, 0x01},
                                                                 {0x01, 0x01, 0x02, 0x03},
                                                                 {0x03, 0x01, 0x01, 0x02}};

const uint8_t sboxtab[256] = {};

/*Gallois Field Multiplication*/
uint8_t gmul(uint8_t a, uint8_t b)
{
    int p = 0;
    int counter;
    int hi_bit_set;
    for (counter = 0; counter < 8; counter++)
    {
        if ((b & 1) == 1)
            p ^= a;
        hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set == 0x80)
            a ^= 0x1b;
        b >>= 1;
    }
    return p;
}
void KeyGen(uint8_t roundkeys[][STATE_ROW_SIZE][STATE_COL_SIZE], uint8_t master_key[STATE_ROW_SIZE][STATE_COL_SIZE])
{
    for (uint8_t i = 0; i < STATE_ROW_SIZE; i++)
    {
        for (uint8_t j = 0; j < STATE_COL_SIZE; j++)
        {
            roundkeys[0][i][j] = master_key[i][j];
        }
    }

    for (uint8_t round = 1; round <= ROUND_COUNT; round++)
    {
        ColumnFill(roundkeys, round);
        OtherColumnsFill(roundkeys, round);
    }
    printf("KeyGen done\n");
    for (int i = 0; i < ROUND_COUNT + 1; i++)
    {
        for (int j = 0; j < STATE_ROW_SIZE; j++)
        {
            for (int k = 0; k < STATE_COL_SIZE; k++)
            {
                printf("%x ", roundkeys[i][j][k]);
            }
            printf("\n");
        }
        printf("\n");
    }
}

void ColumnFill(uint8_t roundkeys[][STATE_ROW_SIZE][STATE_COL_SIZE], int round)
{
    uint8_t temp[STATE_ROW_SIZE];

    for (uint8_t i = 0; i < STATE_ROW_SIZE; i++)
    {
        temp[i] = roundkeys[round - 1][i][STATE_COL_SIZE - 1];
    }

    uint8_t t = temp[0];
    for (uint8_t i = 0; i < STATE_ROW_SIZE - 1; i++)
    {
        temp[i] = temp[i + 1];
    }
    temp[STATE_ROW_SIZE - 1] = t;

    for (uint8_t i = 0; i < STATE_ROW_SIZE; i++)
    {
        temp[i] = sboxtab[temp[i]];
    }

    temp[0] ^= rcon[round - 1];

    for (uint8_t i = 0; i < STATE_ROW_SIZE; i++)
    {
        roundkeys[round][i][0] = roundkeys[round - 1][i][0] ^ temp[i];
    }
}

void OtherColumnsFill(uint8_t roundkeys[][STATE_ROW_SIZE][STATE_COL_SIZE], int round)
{
    for (uint8_t j = 1; j < STATE_COL_SIZE; j++)
    {
        for (uint8_t i = 0; i < STATE_ROW_SIZE; i++)
        {
            roundkeys[round][i][j] = roundkeys[round - 1][i][j] ^ roundkeys[round][i][j - 1];
        }
    }
}
void AddRoundKey(uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE], uint8_t roundkey[STATE_ROW_SIZE][STATE_COL_SIZE])
{
    for (int col = 0; col < STATE_ROW_SIZE; col++)
    {
        for (int row = 0; row < STATE_COL_SIZE; row++)
        {
            state[row][col] = state[row][col] ^ roundkey[row][col];
        }
    }
}

void SubBytes(uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE])
{
    for (int col = 0; col < 4; col++)
    {
        for (int row = 0; row < 4; row++)
        {
            state[row][col] = sboxtab[state[row][col]];
        }
    }
}

void ShiftRows(uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE])
{
    uint8_t temp_row[STATE_COL_SIZE];
    for (int row = 1; row < STATE_ROW_SIZE; row++)
    {
        for (uint8_t col = 0; col < STATE_COL_SIZE; col++)
        {
            temp_row[col] = state[row][(col + row) % STATE_COL_SIZE];
        }
        for (uint8_t col = 0; col < STATE_COL_SIZE; col++)
        {
            state[row][col] = temp_row[col];
        }
    }
}

void MixColumns(uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE])
{

    uint8_t newState[STATE_ROW_SIZE][STATE_COL_SIZE];
    for (int col = 0; col < STATE_COL_SIZE; col++)
    {
        for (int row = 0; row < STATE_ROW_SIZE; row++)
        {
            int val = 0;
            for (int i = 0; i < STATE_COL_SIZE; i++)
            {
                val ^= gmul(mixColumnMatrix[row][i], state[i][col]);
            }
            newState[row][col] = val;
        }
    }
    state = newState;
}

void AESEncrypt(uint8_t ciphertext[DATA_SIZE], uint8_t plaintext[DATA_SIZE], uint8_t key[DATA_SIZE])
{
    uint8_t roundkeys[ROUND_COUNT + 1][STATE_ROW_SIZE][STATE_COL_SIZE];
    KeyGen(roundkeys, key);
    // MessageToState(state, plaintext);
    AddRoundKey(state, roundkeys[0]);
    for (int round = 1; round < ROUND_COUNT; round++)
    {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundkeys[round]);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundkeys[ROUND_COUNT]);
    // StateToMessage(ciphertext, state);
}

int main(int argc, char *argv[])
{
    uint8_t ciphertext[DATA_SIZE];
    uint8_t plaintext[DATA_SIZE] = "Ceci est un test";

    uint8_t key[DATA_SIZE] = {0x2b, 0x28, 0xab, 0x09,
                              0x7e, 0xae, 0xf7, 0xcf,
                              0x15, 0xd2, 0x15, 0x4f,
                              0x16, 0xa6, 0x88, 0x3c};

    AESEncrypt(ciphertext, plaintext, key);
}